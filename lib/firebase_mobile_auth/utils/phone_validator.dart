...
class PhoneValidator {
  ...
  static String format(String phoneNumber, {String countryCode = 'US'}) {
    final digitsOnly = phoneNumber.replaceAll(RegExp(r'[^\d]'), '');
  ...
  static String? extractCountryCode(String phoneNumber) {
    ...
    for (final code in countryCodes.where((c) => c.length == 2)) {
    ...
    for (final code in countryCodes.where((c) => c.length == 3)) {
      if (digitsOnly.startsWith(code) && digitsOnly.length >= 10) {
        return code;
      }
    }
  ...
  static String formatWithCountryCode(String phoneNumber, String countryCode) {
    final cleaned = clean(phoneNumber);
